...
% augmenting state vector x with component x_i: 
% time integral of r-y (output error with respect to the reference) - augmented state vector is xaug
% augmented dynamics:
% xaug_dot=Aaug*xaug+Baug*u+Gaug*r
% y=Caug*xaug
Aaug=[A zeros(3,1);-C 0];
Baug=[B;0]; % % control u input matrix
Gaug=[zeros(3,1); 1]; % reference r input matrix  
Caug=[C 0];
%% ITAE criterion n=4 poles, w0 is the desired bandwidth [rd/s]
% desired poles are (s/w0+.424+-1.263j) and (s/w0+.626+-.414j)
% Ogata segunda edição pg. 240 Tabela 4.2
% vide também arquivo PDF da KNTU a esse respeito 
w0=1*2*pi; % rd/s (Hz->rd/s) selected here is 1 Hz
